...
unsigned int nTargetSpacing = 1 * 60; // BubbleZcoin - 1 minute blocks
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;
static const int64_t nDiffChangeTarget = 1;

unsigned int nStakeMinAge = 1 * 60 * 60; // 1 hours
unsigned int nStakeMaxAge = -1;  // unlimited
unsigned int nModifierInterval = 10 * 60; // BubbleZcoin - time to elapse before new modifier is computed
...
const string strMessageMagic = "BubbleZcoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight+1 <= 1)
    {
        ...
        else if(pindexBest->nHeight+1 >= 2  && pindexBest->nHeight+1 <= 40321)
    {
        nSubsidy = 10 * COIN;
    }
        else if(pindexBest->nHeight+1 >= 40322  && pindexBest->nHeight+1 <= 80642)
    {
        nSubsidy = 5 * COIN;
	}	
        else if(pindexBest->nHeight+1 >= 80643)
    {
        nSubsidy = 2.5 * COIN;
    }
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // BubbleZcoin: check proof-of-stake block signature
...
// BubbleZcoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BubbleZcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
